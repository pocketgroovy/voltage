...
import const
from witches.shop import buy_closet_space
from witches.user import sync_resources_routine
...
class ModelsTestCase(unittest.TestCase):
    ...
    def test_buy_with_coins__get_avatar_item(self):
        c = Client()
        avatar_item2 = AvatarItems.objects.filter(name='Shirt')[0]
        param2 = {'phone_id': 'a1b2c3d4', 'item_type': 1,
                  'item_id': avatar_item2.id, 'quantity': 1}
        c.post('/witches/buy_with_coins/0', param2)
        user = WUsers.objects.get(phone_id='a1b2c3d4')
        user_avatar = UserAvatarItemsInCloset.objects.filter(user_id=user.id)[0]
        self.assertEqual(user_avatar.avatar_item_id, avatar_item2.id, 'avatar id should be ' + avatar_item2.id + ' but got ' + user_avatar.id)

    def test_buy_with_stones__get_avatar_ite(self):
        c = Client()
        avatar_item1 = AvatarItems.objects.filter(name='Baseball Cap')[0]
        param2 = {'phone_id': 'a1b2c3d4', 'item_type': 1,
                  'item_id': avatar_item1.id, 'quantity': 1}
        c.post('/witches/buy_with_stones/0', param2)
        user = WUsers.objects.get(phone_id='a1b2c3d4')
        user_avatar = UserAvatarItemsInCloset.objects.filter(user_id=user.id)[0]
        self.assertEqual(user_avatar.avatar_item_id, avatar_item1.id, 'avatar id should be ' + avatar_item1.id + ' but got ' + user_avatar.id)

    def test_buy_closet_space__closet_count_increased(self):
        other = 'test_buy_closet_space__out_of_sync_error'
        user = WUsers.objects.get(phone_id='a1b2c3d4')
        buy_closet_space(user, 1, user.phone_id, other)
        self.assertEqual(user.closet, 8, 'closet should be 8 but got ' + str(user.closet))


    def test_use_potion__no_error(self):
        c = Client()
        potion1 = Potions.objects.filter(name='t_potion1')[0]
        param2 = {'phone_id': '9876qwer', 'potion_id': potion1.id}
        c.post('/witches/use_potion/0', param2)


    def test_sync_resources_routine__get_client_count_or_max(self):
        user = WUsers.objects.get(phone_id='zaq12wsx')
        sync_resources_routine(user, 3, 4, 10, 5, now-datetime.timedelta(seconds=7250))
        self.assertEqual(user.ticket, 3)

        sync_resources_routine(user, 8, 150, 10, 5, now-datetime.timedelta(seconds=7250))
        self.assertEqual(user.ticket, 5)
